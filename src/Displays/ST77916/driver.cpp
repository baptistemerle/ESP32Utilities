#include "ESP32Utilities/Displays/ST77916/driver.h"
...
void Driver::init(DisplayTxDoneCallback callback, void* callbackArg)
{
  ...
  {
    auto* self = static_cast<Driver*>(userContext);
  ...
  {
    vendor_config.init_cmds =      factoryCommands;
    vendor_config.init_cmds_size = sizeof(factoryCommands) / sizeof(st77916_lcd_init_cmd_t);
  }
  else
  {
...
void Driver::flush(int32_t x1, int32_t y1, int32_t x2, int32_t y2, const void* rawData)
{
...
bool Driver::requiresByteSwap() const
{
  return true;
}
...
Driver::ScreenRevision Driver::probeScreenRevision() const
{
